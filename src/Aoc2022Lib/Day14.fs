...
let drawRockStructure (points: Point list) : Result<Point list, ParseError> =
    ...
    | _ ->
        points
        |> List.pairwise
        |> List.collect (fun (p1, p2) -> drawLine p1 p2)
        |> Ok
...
let buildCave (lines: string list) : Result<Cave, ParseError> =
    ...
    |> List.fold (fun acc result ->
        acc >>= fun cave ->
        result >>= fun points ->
        Ok (Set.union cave (Set.ofList points))
    ) (Ok Set.empty)

let findMaxY (cave: Cave) : int =
    if Set.isEmpty cave then 0
    else cave |> Set.map snd |> Set.maxElement
...
let dropSand (cave: Cave) (maxY: int) (sandStart: Point) : Result<Point option, SimulationError> =
    let rec fall (x, y) =
        if y > maxY then
            ...
            if not (Set.contains below cave) then
                fall below
