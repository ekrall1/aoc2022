...
type Point = int * int
type Sensor = { Position: Point; ClosestBeacon: Point; Distance: int }
...
let parseLine (line: string) : Result<Sensor, ParseError> =
    if System.String.IsNullOrWhiteSpace(line) then
        ...
        if m.Success then
            ...
            Ok { Position = sensorPos; ClosestBeacon = beaconPos; Distance = distance }
        else
            Error (InvalidLine line)
    ...
    let rec merge acc current remaining =
        ...
        | next :: rest ->
            if next.Start <= current.End + 1 then
                ...
                let merged = { Start = current.Start; End = max current.End next.End }
...
let countPositionsInRanges (ranges: Range list) : int =
    ranges
    |> List.sumBy (fun range -> range.End - range.Start + 1)
...
let findDistressBeacon (sensors: Sensor list) (maxCoord: int) : Point option =
    let isValidPosition (x, y) =
        ...
        sensors |> List.forall (fun sensor -> 
            manhattanDistance sensor.Position (x, y) > sensor.Distance)
